#ifndef _fg_test_2_h_
#define _fg_test_2_h_
#include "common.h"
#include "sbm.h"
...
namespace fragtest2
{
	...
	void init()
	{
		...
		aspect = float(current_height) / float(current_width);
		...
		{
			{ GL_VERTEX_SHADER, ".\\shader\\shadowmap_shadow.vs.glsl" },
			{ GL_FRAGMENT_SHADER, ".\\shader\\shadowmap_shadow.fs.glsl" },
			{ GL_NONE }
		};
		...
		{
			{ GL_VERTEX_SHADER, ".\\shader\\shadowmap_scene.vs.glsl" },
			{ GL_FRAGMENT_SHADER, ".\\shader\\shadowmap_scene.fs.glsl" },
			{ GL_NONE }
		};
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH32F_STENCIL8, DEPTH_TEXTURE_SIZE, DEPTH_TEXTURE_SIZE, 0, GL_DEPTH_COMPONENT, GL_FLOAT, 0);
		...
		// Load the object
		//object.LoadFromVBM("F:\\Source\\oglpg-8th-edition\\media\\bunny.vbm", 0, 1, 2);
		sbobj.LoadFromSBM("F:\\Source\\oglpg-8th-edition\\media\\ninja.vbm", 0, 1, 2);
	}
